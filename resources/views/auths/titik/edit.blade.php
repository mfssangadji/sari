...
@section('title','Ubah Titik Reklame')
@section('content')
<div class="card card-default">
   	<form method="post" action="{{route('titik').'/'.$titik->id}}">
   		@csrf
         @method('PATCH')
      	<div class="card-body">
         	<div class="row">
            	<div class="col-md-12">
                  <div class="form-group">
                     ...
                     <select class="form-control select2" required style="width: 100%" multiple="multiple" id="reklame_id" name="reklame_id">
                        @foreach($reklame as $val)
                          @if($val->id == $titik->reklame_id)
                            <option value="{{$val->id}}" selected>{{$val->nama_jenis_reklame}} ({{$val->kategori->nama_kategori}})</option>
                          @else
                  ...
                  <div class="form-group">
                     ...
                     @enderror
                     <input type="text" name="lokasi" placeholder="Lokasi" required id="lokasi" value="{{$titik->lokasi}}" class="form-control @error('lokasi') is-invalid @enderror">
                  </div>
            	</div>
         	</div>
      	</div>
